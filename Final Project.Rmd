...
missing_pattern <- aggr(data_raw, col = c("navyblue", "red"), numbers = TRUE, 
                        sortVars = TRUE, labels = names(data_raw), 
                        cex.axis = 0.7, gap = 3, ylab = "Missing Data Pattern")
...
# Function to reverse scales
reverse_scale <- function(x, max_value) {
  return(ifelse(x %in% excluded_values, x, (max_value + 1) - x))
}

# Apply the custom function to reverse specific scales
data_cleaned[variables_to_reverse_1_4] <- lapply(data_cleaned[variables_to_reverse_1_4], reverse_scale, max_value = 4)
data_cleaned[variables_to_reverse_1_5] <- lapply(data_cleaned[variables_to_reverse_1_5], reverse_scale, max_value = 5)
data_cleaned[variables_to_reverse_0_10] <- lapply(data_cleaned[variables_to_reverse_0_10], reverse_scale, max_value = 10)
data_cleaned[traits_variables] <- lapply(data_cleaned[traits_variables], reverse_scale, max_value = 7)
...
# Generate file paths dynamically and read in all imputed datasets
file_paths <- sprintf("../data/imputed_dataset_%d.csv", 1:10)
imputed_datasets <- lapply(file_paths, read.csv)
